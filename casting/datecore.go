...
func NewHrTimeStr(fr string) (HrTime, error) {
	...
	utl.Block{
		Try: func() {
			hr := ToInt(utl.SubString(fr, 0, 2))
			min := ToInt(utl.SubString(fr, 3, 5))
...
func (p *HrTime) Valid() error {
	if p.hr > 24 || p.hr < 0 {
		return utl.StrErr("la hora es invalida colocar en formato de 24HR")
	}
	if p.min > 59 || p.min < 0 {
		return utl.StrErr("los Minutos es invalida colocar en formato de 59Min")
	}
	return nil
}
...
func (p *HrTime) EqualNow() bool {
	date := time.Now()
	return utl.ReturnIf(date.Hour() == p.hr && date.Minute() == p.min, true, false).(bool)
}
