...
func (p *Worker) StartTime() {
	if !utils.DateEquals(p.fexec, time.Now()) && p.finalid {
		p.finalid = false
	}
...
func NewWork(format, tp string, job Job) (Worker, error) {
	switch tp {
	case "hr", "min":
		tiker, err := casting.NewTicker(format, tp)
		if err != nil {
			return Worker{}, err
		}
		...
		return wrol, nil
	case "time":
		hrtime, err := casting.NewHrTimeStr(format)
		if err != nil {
			return Worker{}, err
		}
		hr, min := hrtime.GetHr()
		wrol, err := NewWorkTime(hr, min, job)
		if err != nil {
			return Worker{}, err
		}
		return wrol, nil
...
func NewWorkTime(hr, min int, job Job) (Worker, error) {
	hrTime, err := casting.NewHrTime(hr, min)
	if err != nil {
		return Worker{}, err
	}
	return Worker{
